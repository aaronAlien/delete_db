        ...
        <div className='flex items-center justify-between gap-3 text-sm'>
          <div className='flex items-center gap-3 flex-1'>
            <div className='flex items-center gap-2'>
              <div
                className={`w-2.5 h-2.5 rounded-full ${users.length > 0 ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
              ></div>
            ...
            <div className='flex gap-4 text-xs text-gray-600 font-semibold'>
              <span>
                users:{" "}
                <strong className='text-accent-600'>{users.length}</strong>
              </span>
              <span>
                cycles:{" "}
        ...
        {!isExpanded ? (
          ...
          >
            <div className='flex items-center gap-3'>
              <div
                className={`w-3 h-3 rounded-full ${users.length > 0 ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
              ></div>
              <span className='font-bold text-gray-900'>📊 db status</span>
            </div>
          </button>
          ...
          <div className='bg-white border-2 border-primary-300 rounded-2xl shadow-2xl w-80'>
            <div className='flex items-center justify-between px-5 py-4 border-b-2 border-gray-100 bg-gradient-to-r from-primary-50 to-accent-50'>
              <div className='flex items-center gap-2'>
                <div
                  className={`w-2.5 h-2.5 rounded-full ${users.length > 0 ? "bg-green-500" : "bg-gray-400"}`}
                ></div>
            ...
            <div className='p-5 space-y-4'>
              ...
              {users.length === 0 ? (
                <div className='bg-gradient-to-r from-primary-50 to-accent-50 border-2 border-primary-200 rounded-xl p-4 text-center'>
                  ...
                  <div className='text-xs text-primary-800 font-bold mb-1'>
                    database empty
                  </div>
                ...
                <div className='bg-gradient-to-r from-accent-50 to-purple-50 border-2 border-accent-200 rounded-xl p-4'>
                  <div className='text-xs text-accent-800 font-bold mb-3'>
                    current users:
                  </div>
                  <div className='space-y-2'>
                    {users.map((user) => (
                      <div
                        key={user.id}
                        className='text-xs text-gray-900 flex items-center justify-between bg-white px-3 py-2 rounded-lg'
                      >
